...
if historical_total > 0:
    ...
    trend = "an increase 📈" if forecast_growth_pct > 0 else "a decrease 📉"
    color = "#ffffff" if forecast_growth_pct > 0 else "#dc3545"  # green or red

    st.markdown(
        f"<p style='color:{color}; font-weight:bold;'>Based on the graph, EV adoption in {county} is expected to show {trend} of {forecast_growth_pct:.2f}% over the next 3 years.</p>",
        unsafe_allow_html=True
    )
else:
...
st.markdown(
    """
    <div style='background-color:#f0fdf4; color:#065f46; padding:12px 20px; border-radius:10px; font-weight:bold; font-size:16px;'>
        ✅ Forecast complete
    </div>
    """,
    unsafe_allow_html=True
...
st.markdown(
    """<div style='font-weight:bold; padding:12px 20px;'> 
            Prepared for the AICTE Internship Cycle 2 by ASWAJ
            </div> 
            """,
            unsafe_allow_html=True)
